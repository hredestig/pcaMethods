...
thus obtain an estimated complete data set. The pcaMethods package war
...
<<>>=
imputed <- pca(mD, nPcs=3, method="ppca")@completeObs
@ 
...
'exprSet' we could simply do
<<>>= 
library(Biobase)
data(exSet)
exSetNa <- exSet
exSetNa@exprs[sample(13000, 200)] <- NA
lost <- is.na(exSetNa@exprs)
impExSet <- pca(exSetNa, nPcs=2, method="ppca")[[2]]
sum((exSet@exprs[lost] - impExSet@exprs[lost])^2) / sum(exSet@exprs[lost]^2)
@ 
