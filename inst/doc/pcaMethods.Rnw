...
All methods use a common class called \texttt{pcaRes} to return the
results. This guarantees maximum flexibility for the user. A wrapper
...
estimation results.
For the sample data all methods show similar similar eigenvalues.
One can also see
...
For BPCA, the eigenvalues are scaled differently because the scaling for scores / loadings differs from the other methods. But this is always a linear factor,
see Figure \ref{fig:loadingBPCA}. The order of the eigenvectors remains the
same. I will try to address this issue in future releases.
\begin{figure}
	\centering
...
Two methods for cross validation are coming with the package.
The first called \texttt{Q2} is for internal cross
validation that allows to estimate the level of structure in a data set and to optimise the choice of number of principal components.
Cross validation is performed by either removing random elements of
...
\caption{Boxplot of the \texttt{Q2} results for BPCA, Nipals PCA,
	SVDimpute and PPCA \label{fig:Q2} PPCA and SVDimpute both
	deliver better results than BPCA and Nipals in this example.}
\end{figure}
...
\caption{Boxplot showing the \texttt{NRMSEP} versus the number of
  ...
  recommendable to allow more iterations to obtain better estimates of
...
{\sl{\LaTeX} - Missing value estimation methods for DNA microarrays.}
Bioinformatics. 2001 Jun;17(6):520-5.
\bibitem{feten05} G. Feten and T. Almoy and A.H. Aastveit
{\sl{\LaTeX} - Prediction of Missing Values in Microarray and Use of
...
Matsubara K. and Ishii S. {\sl{\LaTeX} - A Bayesian missing value estimation method for gene expression profile data.} Bioinformatics. 2003 Nov 1;19(16):2088-96.
\bibitem{wold66} Wold H. {\sl{\LaTeX} - Estimation of principal components and
