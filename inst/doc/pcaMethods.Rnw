...
X = TP^T + V
\end{equation}
Where $X$ denotes the observations, $T$ the latent variables or scores,
$P$ the transformation matrix (consisting of the most significant eigenvectors of the covariance matrix)
...
predictions worse (private communication).
...
based on a linear combination of the $k$ nearest neighbours of an incomplete variable (gene). 
The distance between genes is defined as the absolute
...
subset of complete genes.
This is preferable when the number of incomplete genes is relatively small.

The second way is to consider all genes as candidates.
...
legend(x="topright", legend=c("PCA", "PPCA", "BPCA", "SVDimpute","Nipals PCA"), lty=1:5, col=1:5, cex=2, lwd=3)
@
...
values for a certain gene by the variance within this gene. The idea
...
Further, $G$ denotes the set of incomplete genes, $g$ is the number of
incomplete genes. $O_j$ is the set of missing observations in
gene $j$ and $o_j$ is the number of missing observations in gene $j$.
$\hat{x}_{ijk}$ stands for the estimate of value $i$ of gene $j$ using $k$
loadings. See Figure \ref{fig:kEstimate} for an example. 
<<echo=true, results=hide>>=
errEsti <- kEstimate(md, method = "svdImpute", evalPcs=1:5, nruncv=1)
@
\begin{figure}[!ht]
  \centering
...
\caption{Boxplot showing the \texttt{NRMSEP} versus the number of
  loadings. In this example only one iteration was performed, it is
...
plotPcs(resPPCA, pcs=1:3, type="scores")
@
