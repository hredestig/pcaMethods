...
##'
##' Measures how well described the observations are. High DModX observatios
##' poorly described by the model. Defined as:
##'
...
##' @param trainingData the original training data, taken from \code{completeObs} if
##' left missing.
...
setMethod("DModX", "pcaRes",
          function(object, trainingData, newdata,
                   type=c("normalized", "absolute"), ...) {
              type <- match.arg(type)
              if(missing(trainingData)) {
                  if(!is.null(completeObs(object)))
                      trainingData <- completeObs(object)
                  else stop("missing data when calculating DModX")
              }
              A0 <- as.integer(centered(object))
              if(!missing(newdata)) {
                  E2 <- resid(object, newdata)^2
                  ny <- sqrt(nObs(object) / (nObs(object) - nP(object) - A0))
              } else {
                  E2 <- resid(object, trainingData)^2
                  ny <- 1
              }
              s <- sqrt(rowSums(E2) / (nVar(object) - nP(object))) * ny
              if(type == "absolute")
                  return(s)
              s0 <- sqrt(sum(resid(object, trainingData)^2) / ((nObs(object) - nP(object) - A0) *
