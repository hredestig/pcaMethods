##########################################################################################
##
## ppca <- function(Matrix, nPcs, center = TRUE, seed = NA, ... ) {
##
...
ppca <- function(Matrix, nPcs=2, center=TRUE, seed=NA, ... ) {
  ...
  completeObs <- Matrix
  completeObs[hidden] <- Ye[hidden]

  ## Calculate R2cum
  R2cum <- NULL
  scaled  <- scale(completeObs, center = TRUE, scale = FALSE)
  for (i in 1:ncol(C)) {
  ...
  result@center <- apply(completeObs, 2, mean)
